...
import type { DummyJobOpening, DummyParsedProfile } from '../../data/dummyCareerData.js'
...
export class DiscoveryService {
  ...
  async scrapeProfileWithLinkup(input: {
    linkedinUrl: string
    targetRole?: string
    location?: string
  }): Promise<Partial<DummyParsedProfile> & { scrapedSummary: string; profileSourceUrl: string }> {
    const page = await this.linkupClient.fetchWebpage({ url: input.linkedinUrl })

    const content = page.content.slice(0, 6000)
    const extractedSkills = extractSkills(content)
    const title = extractTitle(content) ?? input.targetRole ?? 'Software Engineer'

    return {
      title,
      targetRole: input.targetRole ?? title,
      targetLocation: input.location ?? 'Remote',
      skills: extractedSkills,
      scrapedSummary: content.slice(0, 500),
      profileSourceUrl: page.url,
      confidence: extractedSkills.length > 0 ? 0.82 : 0.64,
    }
  }
